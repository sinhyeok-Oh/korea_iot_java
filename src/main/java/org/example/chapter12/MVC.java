...
* - 사용자가 인터페이스를 설계하는 스프트웨어 디자인 패턴 중 하나
* - 세 가지 구성 요소로 나누어 개발을 진행( 역할 분담)
*
* cf) 디자인 패넡: 소프트웨어 디자인 가정에서 자주 발생하는 문제들에 대한 일반적인 해결책
*
* 1. Model(모델)
*       : 애플리게이션의 데이터와 비즈니스 로직을 담당
*       - DB, 파일 시스템, 외부 서비스와의 통신을 통해 데이러를 저장하고 검색
*       - 상태 변경에 대해 View와 Controller 에게 알릴 수 있는 메커니즘을 제공
*       >> View 와 Controller 의 세부 구현사항으로 부터 독립적
*
* 2.View(뷰)
...
//      >> Controller 가 View 에 전달 >> View가 화면 출력
