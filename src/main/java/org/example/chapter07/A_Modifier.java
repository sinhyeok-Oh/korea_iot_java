...
import org.example.othePackage.*;
// : * 기호는 전체를 의미
// - 다만, 프로그램의 향상성을위해 권장하지 않음

// == 접근제한자(제어자) == //
// : 클래스, 변수, 메서드의 접근 범위를 제어
// - 외부로부터 데이터르 보호, 객체 간의 의존성을 최소화하는 역할
// - 코드의 보안성을 향상, 갭슐화 구현
...
// cf) default class DefaultClass {} - default 키워드를 직접 명시x
...
class PrivateClass {
    private int myField;
    private void setMyField() {
        System.out.println(myField); // private  필든는 PrivateClass 내부에서만 사용 가능!
    }
}
...
// 3) protected
// : 동일한 패키지 내의 모든 클래스에서 접근 가능(default 와 동이 )
// : 다른 패키지에 있는 경우, 해당 클래스를 상속받은 하위 클래스에서만 접근가능
