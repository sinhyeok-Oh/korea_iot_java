...
*
* 1. 제네릭 장점
* - 타입 안전성: 컴파일 시 타입 체크 가능, 타입 에러 방지
* - 코드 재사용성: 하나의 코드로 다양한 타입 사용 가능
* - 가독성: 사용 시 타입 명시로 코드의 의도 파악 가능
*
* 2. 제네릭 사용 방법
*
*
...
*              , 해당 기능 사용시 작성되는 실질적인 타입이 전달
*       <T>: Type 약자 (가장 흔하게 사용)
*       <K, V>: Key, Value 의 구조 (Map 구조에서 주로 사용)
...
public class A_Generic {
    public static void main(String[] args) {
        ...
        // cf) 제네릭 타입 명시 필수 x
        //        : 제네릭 타입의 구조에 값이 들어올 경우 해당 겂으로 자동 유추
        ...
        // cf) 제네릭 타입은 컴파일 이후 실제 타입 정보가 사라지고 Object 로 변환
        //      - 런타임 시에는 Object 로 동작
        //      >> 타입 소거(Type Erasure)
        List<String> list = new ArrayList<>();
        list.add("Hello");
        //  컴파일 시 리스트 내부 요소가 String 인지 검사
        // >> 런타입 시에는 List<String>이 List로 취급됨 (타입 정보는 사라짐)
