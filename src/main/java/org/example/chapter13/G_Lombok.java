...
// 3. 생성자 어노테이션
// cf) 매개볍수 parameter(params), 인자(인수) argument(args)

// 1) NoArgsConstructor: 인자가 없는 생성자
// 2) AllArgsConstructor: 모든 필드값을 인자로 가지는 생성자
// 3) RequiredArgsConstructor: final 또는 @NonNull 어노테이션이 붙은 필드를 인자로 가지는 생서자
...
// : 두 객체의 내용이 같은지(동등성) 비교 & 구 객체가 같은 객체인지(동일성) 비교 연산자
// > boolean 값 반환

// 5. @Builder 어노테이션
// : 복잡한 객체 생성 시 사용하는 빌더 패턴을 자동으로 구현
// - 필드값을 모두 매서드 체이닝으로 가져옴 ( 필수값 또한 메서드 체이닝으로 구성)
// - 메서드 체이닝 이름은 필드명
// >> 필드값을 체이닝에서 누락 시 오류
...
public class G_Lombok {
    public static void main(String[] args) {
        LombokClass lombokClass1 = new LombokClass("자바는 노잼", "이승아", "코리아");
        ...
        LombokClass lombokClass3 = new LombokClass.LombokClassBuilder()
                ...
                // : Lombok 이 내부에서 빌더 전용 생성자를 생성 초기화를 우회 (예외)
                .title("자바자바자바")
                // @Builder: @NonNull 이 있는 필드에 대해 값을 할당 하지 않으면 NullPointerException 발생
                .author("조승범")
                .build();
