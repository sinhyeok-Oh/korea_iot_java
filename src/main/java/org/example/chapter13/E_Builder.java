...
* - 필드(속송)가 많거나, 필드에 필수값과 선택값이 석여 있을 때 유용
*
* 1. 사용 목적
*   : 생성자가 너무 많아질 때
*   코드가 복잡하고 읽기 어려울 때
*
* 2. 장단점
*   1) 장점: 각독성 향상, 유연성 증가, 확장성 용이
*   2) 단점: 구현 복잡성 증가, 작은 객체 부적합
*
* 3. 동작 원리
*   1) 객체 생성 시 필요한 데이터를 저장하는 Builder 클래스를 생성
*   2) Builder 클래스는 원하는 속성을 단계별로 설정하 수 있는 메서드를 제공
...
//    }
  ...
  // 1) Builder 클래스 정의 (Pizza 클래스 내부의 클래스)
    public static class Builder {
        ...
        // 선택 필드: 기본값 설정( 추가하지 않으면 해당 값으로 이벽)
...
public class E_Builder {
    public static void main(String[] args) {
        ...
        Pizza optionalPizz2 = new Pizza.Builder("쉬립프 피자", "L")
                .addPepperoni()
                .build();

        // cf) 메서드 체이닝
        //      : 메서드 호출 뒤 반환되는 데이터에 곧바로 추가 메서드를 호출
        //      - 객페를 연결고리로 메서드(함수)를 지속적으로 호출
    }
}
