...
public class B_Generic {
    public static void main(String[] args) {
        ...
        Basket<Integer> integerBasket = new Basket<>(); // 숫자를 답는 바구니
        ...
        // Number: 숫자 타입의 Wrapper 타입의 상위 클래스(Integer, Double, Float 등: Number 의 자손)
        List<? extends Number> numbersList = new ArrayList<>();
        // : Number의 자손 타입 중 하나이긴 하나, 정확히 어떤 타입인지 확인 불가!
        // >> 읽기는 가능!, 쓰기는 제한!
        Number num = numbersList.get(0); // 읽기는 가능
        // numbersList.add(10); // 컴파일 에러 발생 - 구체적인 숫자 값 추론 불가 ( 타입 안정성 보장 불가)
        ...
        // >> Integer(정수), 정수가 담길 수 있는 상위 타임을 허용(Integer, Number, Object 등)
        integerList.add(123);
        integerList.add(7777);
        // integerList.add(123.456); - Double

        // cf) List<? super Integer) 의미
        //      : 이 리스트는 Integer 의 상위 타입 중 하나로 선언된 리스트
