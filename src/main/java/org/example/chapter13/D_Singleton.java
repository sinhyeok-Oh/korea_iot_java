...
// - 클래스의 인스턴스가 단 하나만 존재
// - 인스턴스에 대해 전역 접근을 제공
// - 주로 설정 정보 관리 객체, 공유 지원 관리 객체, 로그 객체 등에서 사용
...
//      : 일관된 상태 유지
// - 단점
//      : 결합도 증가 - 여러 클래스가 같은 인스턴스 공유시, 하나가 변경될 때 다른 곳에 영향을 미칠 수 있음
...
class Singleton {
    ...
    // 싱들톤 패턴의 전역 접근성을 제공하는 메서드 정의 (인스턴스화 담당)
...
class SchoolPrincipal {
    ...
    // : 해당 클래스로 생성된 객체가 없는 경우에만 새롭게 생상하여 반환
    public static SchoolPrincipal getInstance() {
        if (instance == null) {
            instance = new SchoolPrincipal();
        }
        return instance; // 기존 객체(if 가 false) OR 생성 객체(if가 true)반환
    }

    public void announce() {
        System.out.println("공부 열심히하고 운동 열심히 하고 일찍 자고 일찍 일어나세요~~~~~");
    }
}
