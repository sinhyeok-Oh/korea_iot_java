...
// -  SRP, OCP, LSP, ISP, DIP
// - 소프트웨어 설계에서 성을 높임유지보수성과 확장
// >> 다양한 디자인 패턴(Design Pattern) 이 SOLID 설게 원칙예 따라 만들어짐
...
//      - 상속, 캡슐화, 추상화, 다형상은 OOP의 4가지 특성!

// 1.SRP(Single Responsibility Principle, 단일 책임 원칙)
// - 클래스는 단 하나의 책임만을 가져야 함
// - 한 가지 역할( 기능) 만 담당 / 하나의 변화 이유만을 가져야함
...
// 2) 올바른 설계 예시 - 각각의 클래스가 자신의 열할만을 담당 ( 유지보수와 테스트가 쉬워짐 )
class Janitor { // 관리인
    void cleanClassroom() {
        System.out.println("교실을 첨소합니다.");
    }
}
...
// 1) 잘못된 설계 예시 - 새로운 학생 유형이 생길 때마다 if 문 추가 (기본 코드 수정 필요)
class LunchMenu {
...
// 변화가 발생해도 LunchMenuInterface 를 구현한 클래스만 추가 - 확장만 하면 됨
...
// 1) 잘못된 설계 예시 - Student의 동작과 InjuredStudent 가 같은 메서드를 통해 서로 다른 생동을 실행함
class Student {
    void playSoccer() {
