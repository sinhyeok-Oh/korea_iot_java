...
//    void exercise() {
    ...
    // > 부모 클래스의 메서드를 자식 클래스에서 재정의 했다는 것으로 명시적으로 나타냄

    //@Override
    // cf) 생략해도 오류가 나지않는 이유
    // : 자바 컴파일러가 메서드 시크니처(반환타입, 이름, 매개 변수) 를 비교하여
...
// 2. 매서드명 바로 위에 @Override 어노테이션을 작성
// 3. 다향성 구현
// : 하나의 이름으로 여러 기능을 수행하는 코드 작성
// 4. 재사용성, 확작성 향상
// : 기존의 코드 변경 없이 새로운 기능 우가와 변경 가능
...
public class D_Overriding {
    public static void main(String[] args) {
        ...
        // >> 해당 상태나 동작을 반드시 가지지만
        //      , 상속받는 클래스마다 차이르 두고싶은 경우 - 오버라이딩을 활용

        // == 오버로딩(Overloading) VS 오버라이딩(Overriding) == //
        // : 메서드의 미름을 재서용하는 방법( 다형성 - 같은 이름으로 다른 행위)
