...
*
* === 내부 클래스 사용 목적 ===
* 1. 코드 가독성 향상 - 코드의 구조 파악 용의
* 2. 캡슐화 강화 - 외부의 공개할 필요가 없는 클래스는 내부로 감춤
* 3. 외부 클래스 맴버 접근 - 내부 클래스는 외부 클래스의 필드와 메서드에 직접 접근 가능
*
* === 내부 클래스 종류 ===
...
*   >> 행당 메서드 내에서만 사용 가능
*   >> 메서드의 지역 변수에만 접근 가능
*   cf) 로컬 변수(외부 클래스의 멤버 변수)는 final인 경우에만 접근 가능
*
* 4. 익명 (내부) 클래스
*   : 이름이 없는 내부 클래스
...
class OuterClass {
    ...
    static class StaticClass {
        void display() {
            ...
            // System.out.println("외부 클래스 필드에 접근: " + outerField);
            // 외부 클래스의 정적 멤버 접근
            System.out.println("외브 클래스 정적 필드에 접근 " + staticField);
        }
    }

    // 3. 메서드 내부 클래스 (지역 클래스)
    void outerMethod() {
        // cf) 지역변수는 암묵적으로 final 로 간주(생략 가능)
        //      >> 재할당이 이루어지면 final 이 사라짐(final로 취급X)
...
public class A_Inner {
    public static void main(String[] args) {
        System.out.println("=== 비 정적 내부 클래스 ===");
        // 1) 외부 클래스 인스턴스화
