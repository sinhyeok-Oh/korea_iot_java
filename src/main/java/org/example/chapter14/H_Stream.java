...
public class H_Stream {
    public static void main(String[] args) {
        ...
        //      cf) 문자열.contains("문자열"): 해당 문자여 포함 여부를 반환( boolean)
        ...
        // 2)
        System.out.println("이름 길이가 3글자인 사람 출력");
        names.stream()
...
//                .forEach(System.out::println);
        names.stream()
                // 내림 차순 (숫자가 큰 수 부터 정열)
                // a, b는 리스트 안의 두 문자열
                // : b.length() - a.length()는 문자열 길이 차이르 이요하여 정렬 순서 결정
                // >> b가 더 크면 양수
                // >> 더 큰 값을 앞으로 정렬(양수면 b가 먼저, 음수면 a가 먼저, 0이면 그대로)
                .sorted((a, b) -> b.length() - a.length())//오름 차순 정렬 (기본)
                .forEach(System.out::println);
